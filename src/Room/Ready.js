...
module.exports = (gamef, bot, joinID) => {
    ...
    if (userRoom != undefined) {
        ...
        if (!joinUser.ready) {
            ...
            roomChatAll(bot, gamef.getRoom(userRoom).players, 0, `${user.first_name} đã sẵn sàng! (${gamef.getRoom(userRoom).readyCount}/${gamef.getRoom(userRoom).players.length})`);
            gamef.gameIsReady(userRoom, (gameReady) => {
                if (gameReady && !gamef.getRoom(userRoom).ingame) {
                    ...
                    roomChatAll(bot, gamef.getRoom(userRoom).players, 0, `Tất cả mọi người đã sẵn sàng! Game sẽ bắt đầu...\n${roleListTxt}\n🌛Đêm thứ ${gamef.getRoom(userRoom).day}🌛`);
